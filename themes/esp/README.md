...
| 功能 | 入口 | 說明 |
| ---- | ---- | ---- |
| 維護總覽 | 右上角個人選單 →「維護總覽」（具文件管理權限或 Admin 可見） | 匯總所有頁面的維護人、週期、狀態與下一次到期時間。Admin 另可查看最後審核時間，便於總管維護週期；選單會以紅色數字徽章提示待處理量。 |
...
- **維護人編輯頁面**：只要維護人對 Page 進行編輯並儲存，系統會自動把維護狀態切換為「待審」，並通知管理員審核該修訂版本（可於內建修訂歷史檢視差異）。待審數量會同步顯示在頁首日曆圖示及個人選單的紅色徽章上。
- **管理員編輯頁面**：若管理員直接編輯並儲存，系統會把狀態標記為最新版本並重新計算下一次到期日，免除額外審核步驟。
- **審核通過 / 駁回**：管理員可在維護卡片或「維護任務總覽」中直接通過或駁回；通過後重新計算維護週期並通知維護人，駁回則保留「更新中」狀態，維護人再次編輯即可重新送審。
...
- 「即將到期」的門檻預設為 7 天，可視需求調整 `MaintenanceService::DUE_SOON_THRESHOLD_DAYS` 常數。
  - 郵件寄送會套用 BookStack `.env` 的 SMTP 設定（例如 `MAIL_DRIVER=smtp`、`MAIL_HOST=...` 等），與其他系統通知共用一套信件來源設定。
